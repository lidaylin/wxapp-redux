...
const server = require('./server/server.json')
const mock = require('./server/mock.js')
 ...
 */
gulp.task('lint', () => {
  return gulp.src(['*.{js,json}', '**/*.{js,json}', '!node_modules/**', '!dist/**', '!**/bluebird.js', '!server/*.{js, json}', '!server/*.json'])
    .pipe(plugins.eslint())
...
gulp.task('watch', ['build'], () => {
  ...
  if (server.isMock) {
    ...
    app.use('/api', httpProxy({target: server.httpProxy, changeOrigin: true}))
  }
  app.listen(server.port, function () {
    console.log('listen this port' + server.port)
  })
})
